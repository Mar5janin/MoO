...
//    Zmniejsza przydziały populacji jeśli suma przekracza ogólną populację planety
    ...
    public void addBuildingToQueue(BuildingType type, ResearchManager researchManager) {
        ...
        // Przywróć zapisany postęp lub zacznij od pełnego kosztu
    ...
    public void removeFromQueue(int index) {
        if (index >= 0 && index < productionQueue.size()) {
            ...
            // Zapisz postęp tylko jeśli coś już zbudowano
    ...
    public Ship processTurn(StarSystem system) {
        ...
        // System głodu - przy ujemnej produkcji żywności populacja umiera
        if (netFoodProduction < 0) {
            foodAccumulated += netFoodProduction;

            // Śmierć od głodu: każde -10 jedzenia zabija 1 osobę
            if (foodAccumulated <= -10 && totalPopulation > 1) {
                int deaths = (int)(Math.abs(foodAccumulated) / 10);
                deaths = Math.min(deaths, totalPopulation - 1);  // Zawsze zostaw 1 osobę
            ...
            double foodNeeded = getFoodForGrowth();
            ...
            if (foodAccumulated >= foodNeeded && totalPopulation < getMaxPopulation()) {
                ...
                // Ogranicz akumulację jedzenia gdy populacja jest na max
    ...
    public double getFoodForGrowth() {
    ...
    public int getMaintenanceCost() {
        ...
        // Kara za okupowaną planetę (nie należy do oryginalnego właściciela)
